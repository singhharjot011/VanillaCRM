...
class RenderModal extends Views {
  ...
  addHandlerModal(
    ...
    if (curHashId === "new-client") {
      renderCreateClientModal(null, this._parentElement, this._usersData);
    }

    if (curHashId === "new-task") {
      renderCreateTaskModal(null, this._parentElement, this._usersData);
    }
    ...
    clientForm?.addEventListener("submit", (e) => {
      ...
      const newConsultant = formMap.get("client-consultant");
      ...
      if (curHashId === "new-client") {
        ...
        const newClientObj = {
          ...
          createdBy: "66bbf5d9a02e1d270d0189c0",
        ...
        const originalData = this._currentData;
        ...
        originalConsultant.name === newConsultant ||
        ...
        updatedClientObj = {
          ...
          consultantName: newConsultant,
          clientNote: newClientNote,
          lastUpdatedBy: "66bd6daf18b4120ed48ad221",
    ...
    taskForm?.addEventListener("submit", (e) => {
      ...
      const assignedToName = formMap.get("assign-to");
      const requestedByName = formMap.get("requested-by");
      ...
      const clientName = formMap.get("client");
    ...
    caseForm?.addEventListener("submit", (e) => {
      ...
      const newCaseConsultant = formMap.get("case-consultant");
      const newCaseNote = formMap.get("case-note");
      const newClientName = formMap.get("client");
      ...
      if (curHashId === "new-case") {
        ...
        const newCaseObj = {
          ...
          consultantName: newCaseConsultant,
        ...
        !newClientName && missingFields.push("Client Name");
        ...
        const {
          ...
          assignedTo: originalCaseConsultant, // Use assignedTo from backend instead of consultantName
        ...
        originalCaseConsultant.name === newCaseConsultant ||
        ...
        // If no values are updated, return early
        if (updatedValues.length === 0) return;

        // Prepare the updated case object
        updatedCaseObj = {
          ...originalData,
          caseType: newCaseType,
          caseStatus: newCaseStatus,
          caseDescription: newCaseDescription,
          assignedToName: newCaseConsultant, // Ensure the backend expects a user ID here
          notes: isNoteUpdated
            ? [
                ...originalData.notes,
                {
                  note: newCaseNote,
                  writtenByName: newCaseConsultant, // Ensure this is a valid user ID for backend
                  writtenAt: new Date().toISOString(),
                },
              ]
            : originalData.notes,
        ...
        // Pass the updated case object to the handler for backend processing
